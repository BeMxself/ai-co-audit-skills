...
usage() {
  cat <<'USAGE'
Usage:
  automation/implementation-audit-kiro/run-implementation-audit.sh --task-dir <task-dir> [options]
...
Options:
  ...
  --dry-run              Generate prompts/artifacts without invoking kiro-cli/codex
  -h, --help             Show help
USAGE
}
...
if [[ "$DRY_RUN" -eq 0 ]]; then
  if [[ -z "$CLAUDE_CMD" ]]; then
    ...
    KIRO_AGENT_FALLBACK="${IMPLEMENTATION_AUDIT_KIRO_AGENT:-ai-co-audit-kiro-opus}"
    CLAUDE_CMD="prompt=\"\$(cat)\"; kiro-cli chat --agent \"${KIRO_AGENT_FALLBACK}\" --no-interactive --trust-all-tools \"\$prompt\""
  fi
  if [[ -z "$CODEX_CMD" ]]; then
