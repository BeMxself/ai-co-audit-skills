#!/usr/bin/env bash
# sync-automation.sh
# Compares implementation-audit-kiro reference scripts against a target directory and reports/copies differences.
...
resolve_non_cache_reference_dir() {
  ...
  if [[ -n "${AI_CO_AUDIT_SKILLS_SOURCE_DIR:-}" ]]; then
    candidates+=("${AI_CO_AUDIT_SKILLS_SOURCE_DIR%/}/skills/implementation-audit-kiro/automation-ref")
    candidates+=("${AI_CO_AUDIT_SKILLS_SOURCE_DIR%/}")
  fi
  candidates+=("${PWD}/skills/implementation-audit-kiro/automation-ref")
  candidates+=("${HOME}/Projects/ai-co-audit-skills/skills/implementation-audit-kiro/automation-ref")

  if [[ -d "${HOME}/Projects" ]]; then
    while IFS= read -r discovered_repo; do
      candidates+=("${discovered_repo}/skills/implementation-audit-kiro/automation-ref")
...
if ! is_valid_reference_dir "$REFERENCE_DIR"; then
  ...
  echo "[ERROR] Expected implementation-audit-kiro automation files are missing." >&2
  exit 2
fi
