...
base_task_id="$(date '+%Y%m%d')-design-consistency-audit"
mkdir -p "${tmp_dir}/.ai-workflows/${base_task_id}"
...
expected_task_id="${base_task_id}-01"

if [[ "$task_id" != "$expected_task_id" ]]; then
  echo "expected unique task id ${expected_task_id}, got: ${task_id}"
  exit 1
fi
